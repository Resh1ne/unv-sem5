...
class ImplMatrix:
    ...
    def set_sets(self, other: "ImplMatrix") -> None:
        self.first_elems_set = other.first_elems_set
        self.second_elems_set = other.second_elems_set

    def is_appliable(self, set_name: str, sets: Sets) -> bool:
...
def get_impl_matrix(impl: Implication, sets: Sets) -> ImplMatrix:
    matrix: ImplMatrix = ImplMatrix(
        impl.first, impl.first_elem, impl.second, impl.second_elem
    )
...
def apply_set_impl(set_name: str, matrix: ImplMatrix, sets: Sets) -> Set:
    new_matrix: ImplMatrix = ImplMatrix(
        matrix.first_name, matrix.first_elem, matrix.second_name, matrix.second_elem
    )
    set = sets[set_name]
...
def is_unique_set(new_set: Set, sets: Sets) -> Optional[str]:
